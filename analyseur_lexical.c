...
// La dd�finition de la taille des tableaux ci-dessus
...
// une fct li kat2ignor� les espaces w les commentaires...
...
// mn b3d ma9rina caract�re wa7ed b nextchar had la fct kat3iyet les fcts li ghay9raw lina kalma kamla
Token getToken(){
    // vider le token
...
boolean isNumber(){
//ila kan le caract�re lu b nextchar un chiffre kankamlo sinon kanraj3o false l gettoken
...
//7atina le caract�re lewel f num[0]
    num[0] = currentChar;
    i++;

//hnaya kantestew wach lcarat�re tani un pt ==> le cas d'un r�el (par exple 1.5)
...
//hnaya kantestiw wach lcaract�re tani un alphabet ==> (par exple 3la7assab)
    else {
    int t = isdigit(num[i]);
...
// le cas d'un chiffre suivi de caract�re sp�cial
...
//had la partie kan9raw fiha un nbre entier ou r�el kamel
    else if(t!=0 && isalnum(num[i])!=0 ) {
    while( ++i <= MAXDIGIT && isdigit(num[i] = currentChar = NextChar())  );
...
boolean isWord(){
//ila kan le caract�re lu b nextchar un  alphabet kankamlo sinon kanraj3o false l gettoken
...
// le cas ila l9ina le mot f le tableau des mots ccl�s
    if(cmp == 0){
        strcpy(currentToken.name, keywords_list[j-1]);
        strcpy(currentToken.value, word);
    }
// le cas ila makanch le mot f le tableau des mots cl�s ==> donc ra howa un identificateur ou string
    else{
        if(testguill == 0){
        ...
        // hna le cas ila kena d�ja l9ina des guillemets donc ra automatiquement 7na f string
...
boolean isSpecial(){
    int j;
    int i=0,cmp=0;
    char op[3]; // la taille d'un op�rateur quelconque ne d�passe pas 3
...
//hna zedna ( ) { } bach ila tlass9o m3a egal apr�s man3tabrohomch op�rateur w nmchiw n9albo 3lihom f tableau
